...
function dbstart(data) {
    ...
    sStore('originalPuzzle', outt);

    // Save each row to localStorage.
    for (let i = 0; i < xx; i++) {
        sStore(`puzzProgressRow${i}`, JSON.stringify(grid[i]));
    }
...
function clku(evn){
 ...
 drag='n';
  ...
  const sourceRow = gx;
  const destRow = tx;
  sStore(`puzzProgressRow${sourceRow}`, JSON.stringify(grid[sourceRow]));
  if (sourceRow !== destRow) {
      sStore(`puzzProgressRow${destRow}`, JSON.stringify(grid[destRow]));
  }
...
function sav(msg='Click Ok to save this game.',sav=0){
 ...
 if (sav==1) {
  //console.log('progress');
    var temp = ""
    for (let i = 0; i < xx; i++) {
        temp += gStore(`puzzProgressRow${i}`);
    }
 } else {
  //console.log('original');
  temp=outt ? outt: gStore('originalPuzzle');
 }
